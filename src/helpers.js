...
exports.keystrokeForKeyboardEvent = function (event, customKeystrokeResolvers) {
  ...
  if (
    NUMPAD_KEY_NAMES_BY_KEYBOARD_EVENT_CODE[code] != null &&
  ...
  if (isNonCharacterKey) {
    ...
    if (
      ...
      if (process.platform === 'darwin' && event.code) {
        ...
        if (
          nonAltModifiedKey &&
          (ctrlKey || metaKey || !isASCIICharacter(key))
        ) {
  ...
  if (
    ...
    if (
      (characters = slovakCmdCharactersForKeyCode(event.code, currentLayout))
    ) {
      if (event.shiftKey) {
        key = characters.withShift
      } else {
  ...
  if (
    ...
    (shiftKey &&
      event.type !== 'keyup' &&
      (isNonCharacterKey ||
        (isLatinCharacter(key) && isUpperCaseCharacter(key))))
  ) {
    if (keystroke) {
      keystroke += '-'
    }
