  ...
  KeymapManager = class KeymapManager {
    ...
    */
    ...
    build(source, keyBindingsBySelector, priority, throwOnInvalidSelector) {
      ...
      for (var selector in keyBindingsBySelector) {
        ...
        if (
          ...
          !isSelectorValid(selector.replace(/!important/g, ''))
        ) {
          console.warn(
        ...
        for (var keystrokes in keyBindings) {
          ...
          command =
            (left = __guardMethod__(command, 'toString', o => o.toString())) !=
            null
              ? left
              : ''
    ...
    removeBindingsFromSource(source) {
      this.keyBindings = this.keyBindings.filter(
        keyBinding => keyBinding.source !== source
      )
      return undefined
    }
    ...
    */
    ...
    loadKeymap(bindingsPath, options) {
      ...
      if (checkIfDirectory && fs.isDirectorySync(bindingsPath)) {
        for (var filePath of Array.from(
          fs.listSync(bindingsPath, ['.cson', '.json'])
        )) {
        ...
        this.add(
          ...
          this.readKeymap(
            bindingsPath,
            options != null ? options.suppressErrors : undefined
          ),
    ...
    filePathMatchesPlatform(filePath) {
      ...
      for (var component of Array.from(
        path.basename(filePath).split('.').slice(0, -1)
      )) {
    ...
    handleKeyboardEvent(event, param) {
      ...
      if (exactMatchCandidates.length > 0) {
        ...
        while (
          ...
          var exactMatches = this.findExactMatches(
            exactMatchCandidates,
            currentTarget
          )
          for (exactMatchCandidate of Array.from(exactMatches)) {
            ...
            if (
              ...
              eventHandled = true
              for (var pendingKeyupMatch of Array.from(
                pendingKeyupMatchCandidates
              )) {
    ...
    */
    ...
    findPartialMatches(partialMatchCandidates, target) {
      ...
      while (
        partialMatchCandidates.length > 0 &&
        target != null &&
        target !== document
      ) {
    ...
    terminatePendingState(fromTimeout) {
      let bindingsToDisable = this.pendingPartialMatches.concat(
    ...
    simulateBubblingOnDetachedTarget(target, commandEvent) {
      ...
      while (currentTarget != null) {
        ...
        currentTarget =
          currentTarget.parentNode != null ? currentTarget.parentNode : window
      }
    }
  }
